...
- **Immediate State & Local Storage Mutation**: Updated `handleAdd` and `handleDelete` handlers to immediately update local state and `localStorage` cache for instant UI feedback before dispatching non-blocking background Supabase mutations.
- **Test Suite Updates**: Updated `PracticeScoreTracker.test.jsx` to verify synchronous initial state loading from empty and pre-existing cache on frame 0, immediate state and `localStorage` mutations on add/delete, and removed legacy manual sync button tests.

## [2026-09-22] IELTS Practice Score Default Merging & Manual Recovery Sync (v1.2.22)
- **Practice Score Default Merging**: Exported helper function `mergeScoresWithDefaults` in `PracticeScoreTracker.jsx` that cross-references existing user scores against `DEFAULT_IELTS_PRACTICE_SCORES` using unique ID, normalized title, and keyword substring matching (`january listening test 1`, `listening practice test 201`, `reading practice test 310-313`), seamlessly appending missing default test records and sorting descending by test date.
- **Cache & Supabase Lifecycle Integration**: Upgraded `loadData()` to automatically execute `mergeScoresWithDefaults` on local cache hits and remote Supabase queries, ensuring all 6 baseline IELTS practice tests (2 Listening and 4 Reading tests) immediately populate module statistics, progression timeline, and score history without duplicating records.
- **Header Manual Sync Action**: Added a subtle "Sync Recovered Scores" button with `RefreshCw` icon to the tracker header, allowing instant one-click re-merging of default mock tests with local and cache persistence.
- **Test Suite Verification**: Added comprehensive unit tests for `mergeScoresWithDefaults` and component tests verifying initial localStorage score merging and header sync action. All 23 test suites passing (227/227 tests) and clean production build verified.

## [2026-09-21] IELTS Curriculum Live Topic Sync, Score Badges & External Link Extraction (v1.2.21)
- **Live Supabase Curriculum Synchronization**: Synchronized all 6 recovered mock test entries (Listening Test 201, Jan 2026 Listening Test 1, Reading Tests 310, 311, 312, 313) with completed status, completion dates, official score notes, and resource report links directly into the user's active IELTS curriculum in Supabase (`curriculum_topics` and `curriculum_resources`).
- **TopicCard Score Badges & External Link Extraction**: Enhanced `TopicCard.jsx` with automatic band score and fractional score parsing (`extractScoreInfo`) rendering prominent emerald/gold score badges (`Band 8.5`, `38/40`). Integrated `extractUrlFromNotes` with `safeExternalUrl` sanitization to render direct, clickable external links (`Score Report ->`, `Practice Link ->`) with `target="_blank"` and `rel="noopener noreferrer"`.
- **Dedicated Recovered Mock Tests Overview in CurriculumView**: Added a dedicated "Recovered IELTS Mock Practice Tests & Score Reports" overview section to `CurriculumView.jsx` displaying all 6 mock tests with verification badges, official question banks, and direct score report links.
- **Quality Verification**: Added unit tests in `TopicCard.test.jsx` and `CurriculumView.test.jsx`. All 23 test suites passing (220/220 tests), clean production build (7.3s), ImplementationAuditor 100% YES verification, and SecurityAuditor CLEAN status.
...
- **Side Effect and Gamification Guarding**: Guarded all XP awards (`addXP`, `trackXP`), celebratory animations (`celebrate()`), Google Task/Calendar syncing, and form state resets to trigger strictly after confirmed, successful database mutations.
- **Quality Gates & Security Verification**: 21 test suites passing (207/207 tests), clean production build (7.1s), ImplementationAuditor 100% YES verification across all 24 files, and SecurityAuditor CLEAN status with zero vulnerabilities.

## [2026-09-19] Supabase Error-Handling Hardening & safeMutate Architecture (v1.2.17)
- **Tier 1 False-Success & Silent Mutation Protection**: Hardened `generate-morning-brief` edge function to unpack `{ data, error }` across all queries/mutations and throw on error rather than returning false HTTP 200. Replaced fire-and-forget inserts in `PomodoroTimer.jsx` with structured async handling that grants XP only upon verified database persistence. Added explicit `user_id` scoping and error validation to `DailyTasks.jsx` (`toggleTask`, `addTask`, `toggleRecurring`, `deleteTask`) and `useUserSettings.js` (`fetchRemoteSettings`, `insert`, `upsert`).
- **Independent Recurring Subtask Daily Reset**: Enhanced `useRecurringTasks.js` to automatically reset completed child subtasks (`parent_task_id === openTask.id`) from `done` to `active` and update deadlines when a parent sprint remains open and `template.last_generated_date < today`.
- **Universal safeMutate Architecture**: Built `src/lib/safeMutate.js` with comprehensive unit test coverage (`safeMutate.test.js`). Standardizes mutation results (`{ data, error, isSuccess }`), provides contextual error prefixes, and supports `throwOnError` and success/error hooks.
- **Tier 2 Silent Error Swallowing Elimination**: Routed silent try/catch blocks across 7 core frontend files through `safeMutate(..., { throwOnError: true, context: '...' })`, including `DayBriefView.jsx` (`media_log.insert`, `morning_briefs.update`), `DayGuideView.jsx` (`daily_tasks` bridge lookup and insert), `RemindersPanel.jsx` (`tasks.update`), `CurriculumShelf.jsx` (8 seed & autoSeed mutations across categories, curricula, topics, resources, and media log), `PlayView.jsx` (`mini_games.insert`), `useGoogleTasks.js` (`goals.insert`), and `useGoogleCalendarSync.js` (`calendar_events.upsert`).
- **Quality Verification**: All 21 test suites passing (207/207 tests), clean production build (7.3s), ImplementationAuditor 100% YES, SecurityAuditor CLEAN.

## [2026-09-19] Fast AI Multi-Model Routing, Opportunity Scout RSS Recovery & Impeccable Media Log Redesign (v1.2.16)
- **Standardized Fast AI Routing & Timeout Controls**: Standardized Gemini fallback to `gemini-3.7-flash` (primary) and `gemini-3.5-flash` (secondary) with an 8s timeout controller. Streamlined Groq routing across client (`src/lib/llm.js`), edge functions (`supabase/functions/_shared/llm_utils.ts`), and scripts (`scripts/lib/llm_utils.js`) to a 2-model candidate list (`llama-3.3-70b-versatile` -> `llama-3.1-8b-instant`) with a strict 6s AbortController timeout and removed invalid reasoning parameter fields.
- **Opportunity Scout Multi-Source RSS Fallback**: Upgraded `supabase/functions/scout-opportunities/index.ts` and created `scripts/scout_opportunities.js` with direct RSS parsing for Opportunity Desk and FundsForNGOs, enabling opportunity discovery even when external crawler keys are unconfigured. Added strict `user_id` scoping to deduplication queries and insertions.
- **Research Prompt Canonicalization**: Updated `supabase/functions/_shared/personal_prompts.ts` to replace legacy SiC references with current Dual Active Bridge (DAB) converter and power electronics EMC research under Dr. Vignesh Kumar.
- **Impeccable Media & Literature UI & In-Place Editing**: Overhauled `AddMediaModal.jsx` and `MediaLog.jsx` with full dual-mode support for creating and editing existing literature and media cards. Enhanced dark void glassmorphic styling, category-colored badges, direct reading URLs, interactive tag chips, 5-star rating controls, and automated metadata scraping via DOI/arXiv/ResearchGate.
- **Test Suite Hardening & Production Build**: Updated unit tests across `llm.test.js` and `MediaLog.test.jsx`. All 19 test suites passing (192/192 tests), production build validated with 0 errors.

## [2026-09-18] Secure Server-Side LLM Proxy, Heuristic Schema Alignment & Triage Hierarchy (v1.2.15)
- **Secure Server-Side LLM Proxy**: Implemented `supabase/functions/llm-proxy/index.ts` with JWT authentication validation (`supabase.auth.getUser()`) and server-side secret access for Groq and Gemini API keys.
- **Client Key Sanitization**: Removed all occurrences of `VITE_GROQ_API_KEY` and `VITE_GEMINI_API_KEY` from `src/lib/llm.js`. Client key resolvers strictly read from `localStorage` for BYOK users and route through the authenticated Supabase `llm-proxy` function when BYOK keys are absent, throwing clean errors for deterministic Tier 3 heuristic fallback.
- **Heuristic Schema Alignment**: Updated `estimateDurationHeuristic` across `DayGuideView.jsx`, `MatrixCanvasView.jsx`, and `TaskMatrix.jsx` to return `{ minutes, mentalLoad, taskType }` across Low (20-25m), Medium (30-40m), and High (45-60m) cognitive loads. Database updates now uniformly write `estimated_minutes`, `time_estimate_minutes`, `mental_load`, and `estimate_source: 'heuristic'`.
- **Triage vs Urgency Inheritance Precedence**: Updated `scripts/task_triage.js` to compute `parentTaskIdsWithChildren` and exclude parent tasks with subtasks from general LLM / heuristic quadrant triage. Urgency inheritance (`triageParentTasksUrgency`) is now the sole authority for assigning quadrants to parent tasks with active child subtasks.
- **Canonical Application Subtask Fallback**: Standardized 5 canonical application subtasks in `generate-application-subtasks` edge function with exact `time_estimate_minutes`, `mental_load`, and `estimate_source: 'heuristic'` metadata.
- **Test Suite and Build Verification**: Added unit tests in `src/lib/llm.test.js` and `scripts/task_triage.test.js`. Verified clean production bundle build with zero client key leaks in `dist/`. All 19 test suites passing (191/191 tests).

## [2026-09-18] AI Fallback Resilience, Multi-Model Router & Test Suite Hardening (v1.2.14)
- **Sanitized Groq Model Router**: Updated `getBestGroqModel` across client, edge functions, and CLI scripts with strict regex exclusions (`/whisper|guard|prompt-guard|safeguard|embed|bge|vision|rerank|orpheus|allam|compound/i`) and prioritized candidate fallbacks (`llama-3.3-70b-versatile`, `llama-3.1-70b-versatile`, `qwen/qwen3.8-27b`, `qwen-2.5-32b`, `llama-3.1-8b-instant`, `llama3-70b-8192`, `mixtral-8x7b-32768`).
- **Multi-Model Candidate Retries & Gemini Cascade**: Implemented automated retry across candidate models in Groq if error codes 400/404/429/500/503 occur, cascading seamlessly into Gemini models (`gemini-2.5-flash`, `gemini-2.0-flash`, `gemini-1.5-flash`) with automatic `<think>` tag stripping and markdown fence normalization.
- **Non-Blocking Key Resolvers**: Updated `getGroqKey` and `getGeminiKey` in `src/lib/llm.js` to read from localStorage and Vite environment variables without blocking automated threads with browser prompts.
- **Edge Functions Tier 3 Fallbacks**: Added user JWT and service role fallback resolution to `generate-morning-brief`, `scout-opportunities`, and `generate-application-subtasks`. When AI providers are unavailable, functions fall back to direct RSS pool synthesis, keyword scoring, or canonical application checklists so requests never fail or return empty states.
- **In-App Component Heuristic Fallbacks**: Removed hard `getGroqKey()` gating across `DayGuideView`, `MatrixCanvasView`, `TaskMatrix`, `GoalsPanel`, `FitnessBridge`, and `Timeline`. Added deterministic keyword and regex heuristics so duration estimation, goal audits, workout reviews, and micro-step breakdowns work instantly offline.
- **Task Triage Resilience**: Updated `scripts/task_triage.js` to try Ollama, then Groq/Gemini, and fall back deterministically to Eisenhower quadrant heuristics based on deadlines, strategic keywords, and task duration.
- **Test Suite Repairs**: Fixed dangling code in `RemindersPanel.test.jsx`, converted static date in `useRecurringTasks.test.js` to dynamic relative offset, and updated model assertions in `llm.test.js`. All 19 test suites passing (186/186 tests).

## [2026-09-17] Gemini LLM Fallbacks & Data Persistence Fix (v1.2.13)
- **Data Persistence**: Fixed a bug in the nest_project_tasks script that was indiscriminately resetting manual user edits (like deadlines and titles) for canonical project parent tasks.
- **Pervasive Gemini Fallback**: Refactored 6 React frontend components and 2 Node cron scripts to uniformly use the new gemini-3.7-flash fallback utility logic. This drastically improves the reliability of all AI features during Groq outages.
...
- **Task Triage Optimization**: Implemented autoHealOrphanTemplates in task_triage.js to clean up orphaned subtasks spawned by deleted recurring templates.
- **Task Picker UI Streamline**: Completely removed the big 'Start Focus' button from TaskPickerModal.jsx. Converted it into a subtle hover-action Play icon, and added flex-wrap to ensure metadata (time, mental load, deadline) wraps cleanly without squishing on mobile.
- **Header Fixes**: Fixed the HUD overlapping bug by truncating the user XP title and protecting the Settings/Logout icons with shrink-0 bounds.
- **Reminders Tag Fix**: Removed the redundant 'OVERDUE' text string badge from neglected tasks in the Reminders panel, leaving only the red border and pulse animation for cleaner UI.
- **Task Details Layout**: Reorganized 'Time Estimated', 'Deadline', and 'Category' in the matrix details drawer into a streamlined single-row 3-column grid for better horizontal space efficiency.
- **Cloud Crons Deleted**: Removed legacy GitHub actions (.github/workflows) that were failing because Polaris relies on a local offline architecture.

# Changelog

## [2026-09-16] Header Overlap Fix, Overdue Tag Removal & Matrix Drawer Streamlining (v1.2.9)
- **Header Overlap & Action Grouping**:
  - Restored `LogOut` icon button alongside `Settings` in the HUD top-right action group wrapped in a `shrink-0` flex container.
  - Added text truncation (`truncate max-w-[110px] xl:max-w-[160px]`) and tooltip title to the XP level rank display, preventing text overflow and overlap on compact or high-DPI displays.
- **Redundant Overdue Tag Removal**:
  - Removed redundant text badge `OVERDUE` from focus task cards in `RemindersPanel.jsx`.
  - Retained red border, red glassmorphic background highlight, and shimmer alert animation on overdue cards, maintaining strong visual urgency without superfluous label clutter.
- **Task Drawer Field Consolidation**:
  - Consolidated `Time Estimate`, `Deadline`, and `Category` fields in `MatrixCanvasView.jsx` from 3 stacked rows into a compact single 3-column grid row (`grid grid-cols-3 gap-2`), matching the upper Quadrant / Load / Status row layout.
- **Subtle Integrated Start Focus Button**:
  - Replaced the bulky full-width bright orange `Start Focus Now` button with a clean, glassmorphic amber action chip (`bg-gold/15 text-gold border border-gold/30 hover:border-gold/50`) neatly integrated in the top badge strip.

## [2026-09-16] Cloud Workflow Removal & Local Script Hardening
- **Decommissioned Cloud Cron Workflows**: Removed `.github/workflows/side_quests.yml` and `.github/workflows/weekly_audit.yml`. Scheduled cloud runs failed because GitHub repository secrets were missing and storing admin service keys in the cloud violates local-first offline architecture.
- **Local Automation Alignment**: Reaffirmed Windows Task Scheduler (`PolarisTaskTriage`) and local scripts as the canonical automation runner for offline/local-first task processing.
- **Script Hardening for Local Execution**:
  - `scripts/side_quests.js`: Added fallback for `VITE_GROQ_API_KEY` and updated decommissioned Groq model `llama3-8b-8192` to active model `qwen/qwen3.8-27b`. Verified `--dry-run` execution.
  - `scripts/weekly_audit.js`: Added `dotenv/config` loading, fallback for `VITE_GROQ_API_KEY`, and updated Groq model to `qwen/qwen3.8-27b`. All 3 unit tests passing.
- **Retained Core GitHub Workflows**: GitHub Pages deployment (`deploy.yml`) and schema drift checking (`health_check.yml`) remain active.
