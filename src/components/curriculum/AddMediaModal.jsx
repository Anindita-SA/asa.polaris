...
const AddMediaModal = ({ onSave, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-void/80 backdrop-blur-sm p-0 md:p-4" onClick={onClose}>
      <div className="glass border border-amber-500/20 rounded-t-2xl rounded-b-none md:rounded-xl p-6 w-full w-full max-w-full md:max-w-lg max-h-[85vh] overflow-y-auto scrollbar-hide space-y-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="font-display text-base text-starlight">Log Media</h2>
          <button onClick={onClose} className="text-nova/60 hover:text-starlight transition-colors"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-mono text-nova/60 block mb-1">Started</label>
          ...
          <div>
            <label className="text-xs font-mono text-nova/60 block mb-1">Finished</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs font-mono text-nova/60 block mb-1">Recommended by</label>
        ...
        {/* One-liner */}
        <div>
          <label className="text-xs font-mono text-nova/60 block mb-1">One-line takeaway</label>
        ...
        {/* Full review */}
        <div>
          <label className="text-xs font-mono text-nova/60 block mb-1">Full review</label>
