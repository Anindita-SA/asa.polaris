...
const RECOVERED_MOCK_TESTS = [
  {
    id: 'mock_jan_2026_listening_1',
    title: 'IELTS Online Tests: Mock Test 2026 January Listening Test 1',
    category: 'Listening',
    score: '38/40',
    band: '8.5',
    url: 'https://ieltsonlinetests.com/score/60136001',
    linkText: 'Score Report ->',
    desc: 'Official listening sectional mock with auto-graded instant score report.',
  },
  {
    id: 'practice_test_201_listening',
    ...
    url: null,
    linkText: null,
    desc: 'Official Cambridge audio format listening practice test.',
  },
  {
    id: 'practice_test_313_reading',
    ...
    desc: 'Cambridge Book 20 Academic Reading practice test.',
  },
  {
    id: 'practice_test_312_reading',
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {isIeltsCurriculum && (
        <div className="space-y-6 pt-2">
          ...
          {/* Recovered IELTS Mock Practice Tests & Score Reports */}
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-mono text-starlight flex items-center gap-2">
                <Award className="w-4 h-4 text-emerald-400" /> Recovered IELTS Mock Practice Tests & Score Reports
              </h3>
            ...
            <p className="text-[11px] text-nova/60 leading-relaxed">
              Verified test scores and official question bank / report links for completed IELTS preparation mocks:
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {RECOVERED_MOCK_TESTS.map((test) => {
                const safeUrl = safeExternalUrl(test.url)
                return (
                  <div
                    key={test.id}
                    className="glass border border-emerald-500/30 bg-emerald-500/5 p-3.5 rounded-xl flex flex-col justify-between space-y-2.5 transition-all hover:border-emerald-500/50"
                  >
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 font-bold">
                            Band {test.band}
                          </span>
                          <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-amber-500/15 border border-amber-500/30 text-amber-300 font-semibold">
                            Score: {test.score}
                          </span>
                        </div>
                      </div>

                      <h4 className="text-xs font-mono uppercase tracking-wider text-starlight font-bold leading-snug">
                        {test.title}
                      </h4>
                      <p className="text-[11px] text-nova/60 leading-relaxed">{test.desc}</p>
                    </div>
                    ...
                    <div className="pt-2 flex items-center justify-between border-t border-blue-900/15">
                      {safeUrl ? (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-[11px] text-sky hover:text-starlight bg-sky/10 border border-sky/20 px-2.5 py-1 rounded-lg transition-colors font-mono font-medium"
                        >
                          <span>{test.linkText || 'Score Report ->'}</span>
                          <ExternalLink className="w-3 h-3" />
                        </a>
                      ) : (
                        <span className="text-[10px] font-mono text-nova/50 italic">
                          Local Cambridge Audio Drill
                        </span>
                      )}
