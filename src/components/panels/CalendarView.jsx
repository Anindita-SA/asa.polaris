...
const CalendarView = () => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-void">
      {/* Header Toolbar */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 md:p-6 border-b border-pulsar/30 bg-void/70 backdrop-blur-sm z-10 shrink-0 gap-4">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {/* View Tab Switches */}
          <div className="flex glass border border-pulsar/40 rounded-lg p-1">
            <button 
              onClick={() => setActiveTab('gcal')} 
              className={`px-3 py-1.5 rounded-md text-xs font-mono uppercase tracking-wider transition-all ${activeTab === 'gcal' ? 'bg-sky/20 text-sky border border-sky/30' : 'text-nova/60 hover:text-starlight'}`}>
              Google Calendar
            </button>
            <button 
              onClick={() => setActiveTab('backups')} 
              className={`px-3 py-1.5 rounded-md text-xs font-mono uppercase tracking-wider transition-all flex items-center gap-1.5 ${activeTab === 'backups' ? 'bg-sky/20 text-sky border border-sky/30' : 'text-nova/60 hover:text-starlight'}`}>
          ...
          {activeTab === 'gcal' && providerToken && (
            <div className="flex items-center gap-2">
              <button onClick={() => fetchEvents(currentWeek)} disabled={loading}
                className="p-2 rounded-lg hover:bg-stardust/30 text-nova/60 hover:text-starlight transition-colors"
                title="Refresh Calendar">
                <RefreshCcw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
              </button>
              <div className="h-4 w-px bg-blue-900/30 mx-1" />
              <button onClick={() => setCurrentWeek(d => subWeeks(d, 1))}
                className="p-1.5 rounded-lg hover:bg-stardust/30 text-nova/60 hover:text-starlight transition-colors">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={() => setCurrentWeek(new Date())}
                className="px-2.5 py-1 rounded-lg hover:bg-stardust/30 text-xs font-mono uppercase tracking-wider text-nova/60 hover:text-starlight transition-colors border border-pulsar/30">
                Today
              </button>
      ...
      {activeTab === 'backups' ? (
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
          <div className="glass border border-pulsar/40 p-5 rounded-xl space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-emerald-400" />
                <h3 className="font-display text-starlight text-lg">Main Control & Backup Vault</h3>
              </div>
              <button
                onClick={() => createBackup()}
                className="px-4 py-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 rounded-xl text-xs font-mono uppercase tracking-wider flex items-center gap-2 transition-all">
                <Archive className="w-4 h-4" /> Create Snapshot Now
              </button>
            </div>
            <p className="text-xs text-nova/60 leading-relaxed">
              All your schedules are versioned and backed up. When you click <b>"Create Snapshot Now"</b>, Polaris generates an un-deletable version entry in Supabase and triggers a clean <code className="text-sky bg-void/70 px-1 py-0.5 rounded">.Ics</code> file download for your local archives.
            </p>
            ...
            <div className="pt-2 flex items-center gap-4 text-xs text-starlight">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="rounded border-blue-900/40 bg-void/70 text-sky focus:ring-sky"
                />
                <span>Auto-sync Google Calendar to Supabase when active</span>
              </label>
              {lastSyncedAt && <span className="text-nova/60 text-[11px]">Last synced: {lastSyncedAt}</span>}
            </div>
          </div>

          <div className="space-y-3">
            <h4 className="text-lg font-mono text-nova/60">Saved Schedule Snapshots ({backups.length})</h4>
            {backups.length === 0 ? (
              <div className="text-center py-8 glass border border-pulsar/30 rounded-xl text-nova/60 text-xs">
                No saved snapshots yet. Click "Create Snapshot Now" above to generate your first backup.
              </div>
            ) : (
              <div className="space-y-2">
                {backups.map(b => (
                  <div key={b.id} className="glass border border-pulsar/30 p-4 rounded-xl flex items-center justify-between hover:border-sky/30 transition-all">
                    <div>
                      <h5 className="text-lg font-display text-starlight">{b.snapshot_name}</h5>
                      <p className="text-xs text-nova/60 mt-0.5">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-16 scrollbar-hide">
          {!providerToken || error?.includes('reconnect') ? (
            <div className="h-full flex flex-col items-center justify-center p-6 text-center">
              ...
              <p className="font-body text-nova/60 mb-6 max-w-sm text-xs leading-relaxed">
